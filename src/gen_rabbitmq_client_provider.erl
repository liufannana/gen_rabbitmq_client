-module(gen_rabbitmq_client_provider).
...
publish(Name, Key, Payload, Opts) ->
	gen_server:call(Name, {publish, Key, Payload, Opts}).
...
handle_call({publish, Key, Payload, Opts}, _From, State = #state{channel = Channel, status = valid, rules = Rulse}) ->
	Exchange = get_exchange(Key, Rulse),
...
handle_info(connect, State = #state{connection_info = ConnectionInfo}) ->
	{ok, ConnectionPid, ChannelPid} = gen_rabbitmq_connection_mon:connect(ConnectionInfo),
	self() ! declare,
...
internal_publish(Fun, Channel, Exchange, Key, Message, Opts)
  when is_binary(Message) ->
    internal_publish(Fun, Channel, Exchange, Key,
                     gen_rabbitmq_client_util:new_message(Message), Opts).

declare_exchanges(_Channel, [], Exchanges) ->
	{ok, Exchanges};
declare_exchanges(Channel, [{Name, _Opts}|Rest], Exchanges) ->
	Exchange = #'exchange.declare'{exchange=Name},
...
declare_queues(Channel, [{Name, _Opts}|Rest]) ->
	Queue = #'queue.declare'{queue=Name},
...
get_exchange(RoutingKey, Rules) ->
	case dict:find(RoutingKey, Rules) of
		error ->
			{error, no_exchange};
		{ok, Value} ->
			gen_rabitmq_client_util:get_one(Value)
	end.	